      ...
      trial(display_element, trial) {
        ...
        <div id='webgazer-validate-container' style='position: relative; width:100vw; height:100vh; overflow: hidden;'>
          ...
          const validation_display = (pt) => {
              ...
              requestAnimationFrame(function watch_dot() {
                  if (performance.now() < pt_finish) {
                      ...
                      success = calculatePercentInROI(pt_data) > trial.dot_threshold;
                      $('.validation-point').css({'background-color': success ? "green" : "red"});
                      wait(500, function() {
                        $('.validation-point').css({'background-color': '#dd494b'}); // TODO change color
                        cancelGazeUpdate();
          ...
          const validate = () => {
              ...
              //x this.jsPsych.extensions.webgazer.showPredictions(true);
              next_validation_point();
          };
          ...
          function drawValidationPoint_PercentMode(x, y) {
              return `<div class="validation-point" style="width:${trial.point_size}px; height:${trial.point_size}px; border-radius:${trial.point_size}px; border: 1px solid #000; background-color: #333; position: fixed; left:${x}%; top:${y}%;"></div>`;
          }
          function drawValidationPoint_CenterOffsetMode(x, y) {
              return `<div class="validation-point" style="width:${trial.point_size}px; height:${trial.point_size}px; border-radius:${trial.point_size}px; border: 1px solid #000; background-color: #333; position: fixed; left:calc(50% - ${trial.point_size / 2}px + ${x}px); top:calc(50% - ${trial.point_size / 2}px + ${y}px);"></div>`;
          }
          // @ts-expect-error
